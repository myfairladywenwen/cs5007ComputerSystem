...
TEST(DirectoryParser, IndexTheFile_OneFile) {
  ...
  //printf("%d elements\n", NumElementsInLinkedList(offset_list));
  //printf("testline 76\n");
...
TEST(MovieTitleIndex, GetDocumentSet) {
  ...
  Movie* m4 = CreateMovieFromRow(kMovie3);
  AddMovieTitleToIndex(ind, m4, doc_id2, row_id2 + 2);
  EXPECT_EQ(NumElemsInHashtable(ind->ht), 12);


  Movie* m5 = CreateMovieFromRow(kMovie4);
  AddMovieTitleToIndex(ind, m5, doc_id2, row_id2 + 3);
  EXPECT_EQ(NumElemsInHashtable(ind->ht), 14);
  ...
  // check "in"
  EXPECT_EQ(DocumentSetContainsDoc(set, 1), 0);
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);
  // check "sleepless"
  set = GetDocumentSet(ind, "sleepless");
  EXPECT_EQ(DocumentSetContainsDoc(set, 1), 0);
  // check "the"
  set = GetDocumentSet(ind, "the");
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);
  ...
  // check "Foo"
  set = GetDocumentSet(ind, "Foo");
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);

  // check "Jonathan"
  set = GetDocumentSet(ind, "Jonathan");
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);

  // check "Alexandra"
  set = GetDocumentSet(ind, "Alexandra");
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);

  // check "Among"
  set = GetDocumentSet(ind, "among");
  EXPECT_EQ(DocumentSetContainsDoc(set, 42), 0);


  // DestroyDocumentSet(set);   -- do not know why if enable this line, will be mem error, tests still pass
