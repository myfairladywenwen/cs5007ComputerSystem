...
int AddMovieTitleToIndex(MovieTitleIndex index,
  ...
  // get each word in the movie title, store each word in a string array (char* [numFileds])
  ...
  for (int j = 0; j < i; j++) {
    ...
    int result = LookupInHashtable(index->ht,
                          FNVHash64((unsigned char*)token[j],
                                    (unsigned int)strlen(token[j])),
                                   &kvp);
    HTKeyValue old_kvp;

    if (result < 0) {
      // key not found, a.k.a., not yet this word in MovieTitleIndex
    ...
    // add this movie's doc_id and row_id into the DocumentSet
