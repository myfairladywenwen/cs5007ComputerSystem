...
const char* movie_row_improper = "8.8|Moive";
...
/**  I don't think no malloc can run pass free
...
TEST(Movie, CreateFromRow) {
  ...
  ASSERT_EQ(0, strcmp(m1->actor_list[0], "Tim Robbins")); // TODO: Check actors
  ...
  snprintf(row, 1000, "%s", movie_row_improper);
  Movie* m_impr = CreateMovieFromRow(row);
  ASSERT_EQ(nullptr, m_impr);
  DestroyMovie(m1);
}
